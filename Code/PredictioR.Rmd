...
To gain more insight into the mechanism of response and resistance to immunotherapy response, we assess the association of 100 genes in PredictIO signature with immunotherapy survival (OS). Note that only studies with at least 20 patients are included in the following analyses.


```{r gene association os, message=FALSE, fig.width=8, fig.height=4, fig.cap = "IO genes association", fig.align="center"}

source("C:/PredictIO_CodeOcean_gene/PredictIO_package/getGeneAssociation.R")
source("C:/PredictIO_CodeOcean_gene/PredictIO_package/getGeneSigAssociation.R")
source("C:/PredictIO_CodeOcean_gene/PredictIO_package/getMetaAnalysis.R")
source("C:/PredictIO_CodeOcean_gene/PredictIO_package/getGeneSigScore.R")

sig <- qread(paste( "C:/PredictIO_CodeOcean_gene/PredictIO_package/data/signatures/qs/" , "PredictIO" , ".qs" , sep="" ))
genes <- sig$gene
...
The association results for Melanoma Van Allen show that 36\% of genes are associated with immunotherapy survival with $FDR < 0.05$, where about 92\% of them are associated with worse survival.     
...
Additionally, the association of genes in PredictIO signature with immunotherapy response (R vs NR) are assessed.
...
The association results for Melanoma Van Allen show that 44\% of genes are negatively associated with immunotherapy response with FDR$<0.05$.    
...
As an example, the meta-analysis approach is applied for CXCL9 to generalize the association with immunotherapy survival. The result shows that the gene is associated with worse survival ($\text{logHR} = -0.2, p = 0.001$), where the significant heterogenity $I^2 = 0.44$ with $Q\,\,p = 0.04$ across datasets.  
...
The following forest plot displays the aggregated results across datasets as pan-cancer analyses. For each dataset, the log hazard ratio (logHR), 95\% confidence intervals (CIs), and p-values are computed. The blue diamond represents the aggregated results. Horizontal bars represent the 95\% CIs of logHR. 

```{r meta-analysis pan-cancer forestplot, fig.width=8, fig.height=5, fig.cap ="sss", fig.align= 'center'}
...
The following forest plot displays the aggregated results across datasets as cancer-specific analysis. 


```{r meta-analysis per-cancer, fig.width=8, fig.height=9, fig.cap ="ccc", fig.align= 'center'}
...
APM_Thompson is a RNA signature capturing the antigen processing and presentation machinery (APM) activity. This APM signature was a predictor of the response of PD-(L)1 blockade in Lung and Melanoma patients.
...
geneSigScore <- lapply(1:length(expr), function(k){
       ...
       geneSig <- getGeneSigGSVA(dat = expr[[k]], 
                                  sig = qread(paste( "C:/PredictIO_CodeOcean_gene/PredictIO_package/data/signatures/qs/" , signature$Signature[i] , ".qs" , sep="" )),
...
IRG_Yang is a RNA signature capturing the expression level of several Immune Regulatory Genes (IRG). This IRG signature was a predictor of the response of CTLA-4 and PD-1 blockade in published cohorts.

```{r signature score computation weighted mean, message=FALSE, warning = FALSE}

i<- which(signature$Signature == "IRG_Yang")
geneSigScore <- lapply(1:length(expr), function(k){
...
- example for specific method and summarize the results

PredictIO is a RNA signature capturing the expression level of ----.
ADD two colors for validation vs discovery cohorts ---> add in the table of immunotherapy Data
...
```{r Fig5, message=FALSE, warning = FALSE, fig.width=10, fig.height=6, fig.cap ="Distribution of PrdictIO signature score across datasets using specific algorithm", fig.align= 'center'}
...
```{r Fig6, message=FALSE, warning = FALSE, fig.width=8, fig.height=5, fig.cap ="aaaa", fig.align= 'center'}
...
```{r Fig7, message=FALSE, warning = FALSE, fig.width=8, fig.height=9, fig.cap ="bbb", fig.align= 'center'}
...
Compare the performance of signature scores to predict immunotherapy response (R vs NR)
- Do the signature score across all signatures
- Do the association of signature score and response
...
AllGeneSig_os <- lapply(1:nrow(signature[-which(signature$Signature == "PredictIO"), ]), function(i){
...
```{r immunotherapy signatures comparison visualization, message=FALSE, warning = FALSE, fig.width=8, fig.height=4}
