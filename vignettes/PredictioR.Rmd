...
getHRdicho( surv = dat_clin$event_occurred_pfs[ dat_clin$cancer_type %in% cancer_type ] ,
...
getKMplot(surv = dat_clin$event_occurred_pfs[ dat_clin$cancer_type %in% cancer_type ],
          time = dat_clin$survival_time_pfs[ dat_clin$cancer_type %in% cancer_type ],
          time_censor = 24,
          variable =  as.numeric(scale(dat_expr["CXCL9", ])), 
          cutoff = median(as.numeric(scale( as.numeric(dat_expr["CXCL9", ]) ))),
...
cox_os <- lapply(1:length(study_icb), function(i){
       ...
       getGeneAssociationSurvival(dat_icb = expr[[i]],
                                  time_censor = 36,
                                  missing_perc = 0.5,
                                  cutoff_n = 20,
                                  feature = genes,
...
p <- getVolcanoPlot(feature = df$Gene, 
                     coef = df$Coef, 
                     pval = df$Pval, 
                     padj = df$FDR, 
                     cutoff_neg = 33, 
...
logreg <- lapply(1:length(study_icb), function(i){
       ...
       getGeneAssociationLogReg(dat_icb = expr[[i]],
                                missing_perc = 0.5,
                                cutoff_n = 20,
                                feature = genes,
                                study =  names(expr)[i])
...
res_meta <- getMeta(Coef = cox_os[cox_os$Gene == "CXCL9", "Coef"], 
                    ...
                    Cancer_type = do.call(rbind, strsplit(cox_os[cox_os$Gene == "CXCL9", "Study"], split='__', fixed=TRUE))[, 2],
                    Treatment = cox_os[cox_os$Gene == "CXCL9", "Treatment"],
                    feature = "CXCL9", 
                    cancer_spec = FALSE, 
                    treatment_spec = FALSE)
...
############################################################################################################################
############################################# ALL THE FOLLOWING META-ANALYSIS NEED TO BE UPDATED ###########################
############################################################################################################################
...
cox_pfs <- lapply(1:length(study_icb), function(i){
       ...
       getGeneAssociationSurvivalDicho(dat_icb = expr[[i]],
                                  time_censor = 24,
                                  missing_perc = 0.5,
                                  cutoff_n = 20,
                                  feature = "CXCL9",
...
# meta-analysis for all genes across datasets
res_meta <- getMeta(Coef = cox_pfs[, "Coef"], 
                    SE = cox_pfs[, "SE"],
                    Study  = cox_pfs[, "Study"], 
                    Pval = cox_pfs[, "Pval"], 
                    N = cox_pfs[, "N"],
                    Cancer_type = do.call(rbind, strsplit(cox_pfs[, "Study"], split='__', fixed=TRUE))[, 2],
...
getForestplotPanCancer(Coef = cox_pfs[ , "Coef"], 
                       SE = cox_pfs[ , "SE"], 
                       Study  = cox_pfs[ , "Study"], 
                       Pval = cox_pfs[ , "Pval"], 
                       N = cox_pfs[ , "N"], 
...
```{r Fig7, fig.width=8, fig.height=9, fig.cap ="Forest plot (CXCL9, binary median cutoff): cancer-specific analysis", fig.align= 'center'}
     
getForestplotPerCancer(Coef = cox_pfs[ , "Coef"], 
                       SE = cox_pfs[ , "SE"], 
                       Study  = cox_pfs[ , "Study"], 
                       Pval = cox_pfs[ , "Pval"], 
                       N = cox_pfs[ , "N"],
...
geneSigScore <- lapply(1:length(expr), function(k){
       ...
       geneSig <- getGeneSigGSVA(dat_icb = expr[[k]], 
                                 sig = sig,
...
geneSigScore <- lapply(1:length(expr), function(k){
       ...
       geneSig <- getGeneSigMean(dat_icb = expr[[k]], 
                                 sig = sig,
                                 signature_name = signature$Signature[i],
                                 missing_perc = 0.5,
                                 cutoff_n = 20,
...
geneSigScore <- lapply(1:length(expr), function(k){
       ...
       geneSig <- getGeneSigPredictIO(dat_icb = expr[[k]], 
                                      sig = sig,
                                      signature_name = signature$Signature[i],
                                      missing_perc = 0.5,
                                      cutoff_n = 20,
...
geneSig_pfs <- lapply(1:length(expr), function(k){
       ...
       if(sum(!is.na(geneSig)) > 0){
         ...
         res <- getGeneSigAssociationSurvival(dat_icb = expr[[k]],
                                              geneSig = geneSig,
                                              time_censor = 24,
                                              cutoff_n = 20,
                                              study =  names(expr)[k],
...
res <- getMeta(Coef = geneSig_pfs$Coef, 
               ...
               N = geneSig_pfs$N, 
               Cancer_type = do.call(rbind, strsplit(geneSig_pfs$Study, split='__', fixed=TRUE))[, 2],
...
```{r Fig12, message=FALSE, warning = FALSE, fig.width=8, fig.height=9, fig.cap ="Forest plot (APM_Thompson signature): cancer-specific analysis", fig.align= 'center'}
...
AllGeneSig_os <- lapply(1:length(GeneSig_list), function(i){  
  ...
  geneSig_os <- lapply(1:length(expr), function(k){
    ...
    if(signature$method[i] == "specific" & signature$Signature[i] == "COX_IS"){
      ...
      geneSig <- getGeneSigCOX_IS(dat_icb = expr[[k]], 
                                  sig = sig,
                                  signature_name = signature$Signature[i],
                                  missing_perc = 0.5,
                                  cutoff_n = 20,
    ...
    if(signature$method[i] == "specific" & signature$Signature[i] == "IPS"){
      ...
      geneSig <- getGeneSigIPS(dat_icb = expr[[k]], 
                               sig = sig,
                               signature_name = signature$Signature[i],
                               missing_perc = 0.5,
                               cutoff_n = 20,
    ...
    if(signature$method[i] == "specific" & signature$Signature[i] == "IPRES"){
      ...
      geneSig <- getGeneSigIPRES(dat_icb = expr[[k]], 
                                 sig = sig,
                                 signature_name = signature$Signature[i],
                                 missing_perc = 0.5,
                                 cutoff_n = 20,
...
AllGeneSig_meta <- lapply(1:length(AllGeneSig_os), function(j){
  ...
  res <- getMeta(Coef = AllGeneSig_os[[j]]$Coef, 
                 ...
                 N = AllGeneSig_os[[j]]$N, 
                 Cancer_type = do.call(rbind, strsplit(AllGeneSig_os[[j]]$Study, split='__', fixed=TRUE))[, 2],
