...
########## studies Liu__Melanoma and Riaz__Melanoma were removed but will be added soon (i=10 and i=19) 
study_icb <- study_icb[- which(study_icb %in% c("Shiuan__Kidney", "Roh__Melanoma", 
                          ...
                          "Liu__Melanoma", "Riaz__Melanoma"))]

group <- c("Discovery", "Validation", "Discovery", "Discovery", "Discovery", "Discovery",
           "Validation", "Discovery", "Discovery", "Discovery", "Discovery",
...
dat_expr <- as.matrix(assay(expr[["Mariathasan__Bladder"]]))
dat_clin <- colData(expr[["Mariathasan__Bladder"]]) %>% as.data.frame()
...
dat_expr <- as.matrix(assay(expr[["Padron__Pancreas"]]))
dat_clin <- colData(expr[["Padron__Pancreas"]]) %>% as.data.frame()
cancer_type <- names( table( dat_clin$cancer_type )[ table( dat_clin$cancer_type ) >= 20 ] )
...
```{r Fig7b, fig.width=8, fig.height=9, fig.cap ="Forest plot (CXCL9, binary median cutoff): treatment-specific analysis", fig.align= 'center'}
     
forestPlotPerTreatment(coef = cox_pfs[ , "Coef"], 
                       se = cox_pfs[ , "SE"], 
                       study  = cox_pfs[ , "Study"], 
                       pval = cox_pfs[ , "Pval"], 
                       n = cox_pfs[ , "N"],
...
```{r Fig11b, message=FALSE, warning = FALSE, fig.width=8, fig.height=9, fig.cap ="Forest plot (APM_Thompson signature): treatment-specific analysis", fig.align= 'center'}
     
forestPlotPerTreatment(coef = geneSig_pfs$Coef, 
                       se = geneSig_pfs$SE, 
                       study  = geneSig_pfs$Study, 
                       pval = geneSig_pfs$Pval, 
                       n = geneSig_pfs$N, 
