...
geneClinSurvCont <- function(dat.icb, clin.var, time.censor, n.cutoff, study, surv.outcome, clin.name){
...
geneSigSurvDicho <- function(dat.icb, clin.var, time.censor, n.cutoff, n0.cutoff, n1.cutoff, study, surv.outcome, clin.name, method = "median"){
...
geneSigLogReg <- function(dat.icb, clin.var, n.cutoff, study, clin.name){
    ...
    if( !( cancer_type %in% "Lymph_node" ) & length(dat_clin$response) >= n.cutoff ){
        ...
        fit <- glm( x ~ clin.var , family=binomial( link="logit" ) )
